...
const BranchSelector: React.FC<BranchSelectorProps> = ({ className = '' }) => {
  const { currentOutlet, setCurrentOutlet, assignedOutlets } = useOutlet();
  const [isOpen, setIsOpen] = useState(false);

  const handleSelectOutlet = useCallback((outlet: OutletSummaryDTO) => {
  ...
  // Don't render if user has no assigned outlets
  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        className="flex items-center space-x-2 bg-blue-700 px-3 py-1.5 rounded-full hover:bg-blue-800 transition-colors text-white"
      >
        ...
        <span className="text-sm font-medium truncate">
          {currentOutlet?.name || 'Select Branch'}
        </span>
        <svg
